...
//
func (wh *WaHandler) HandleLiveLocationMessage(message wa.LiveLocationMessage) {
...
if  receiveTimeDef  > allowTimeDef  {
	...
	liveLocationMessage := models.MessageLiveLocation{
		ContextInfo:                       &messageContext,
		MessageInfo:                       &messageInfo,
		LatitudeDegree:                    message.DegreesLatitude,
		LongituteDegree:                   message.DegreesLongitude,
		AccuracyMeters:                    int32(message.AccuracyInMeters),
		SpeedMPH:                          float64(message.SpeedInMps),
		DegreesClockwiseFromMagneticNorth: int32(message.DegreesClockwiseFromMagneticNorth),
		Caption:                           message.Caption,
		Thumbnail:                         thumbnailURL,
	}
	messageBytes, err := json.Marshal(liveLocationMessage)
	if err != nil {
		log.Errorf("Location Request Json Marshal failed with error %v", err)
		return
	}
	err = Retry(retrySend, time.Millisecond*500, func() error {
		return requestWith(fmt.Sprintf("%v/message/livelocation", *wh.Hook), "application/json", bytes.NewReader(messageBytes))
	})
	if err != nil {
